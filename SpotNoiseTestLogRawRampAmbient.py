...
with open(fileName, 'w') as hFile: # Use hfile to refer to the file object
	...
	while (elapsedTimeHours < totalDurationHours):
		...
		for w in range(0, (2*nAmbients)-1):
			if(thisAmbient >= maxAmbient):
				bRampUp = True
				rampSign = -1
			if(thisAmbient <= minAmbient):
				bRampUp = False
				rampSign = +1
			...
			if(thisAmbient > maxAllowedAmbient):
				print("Tried to exeed maximum temperature")
				thisAmbient = startAmbient
			
			if(thisAmbient < minAllowedAmbient):
			...
			for n in range(0, nfurnace ):
				...
				for k in range(0,spotNRepeats):
					...
					brightMean = np.mean(brightValues)
					darkMean = np.mean(darkValues)
					...
					if count == 0:
						count = count + 1
						arrayOut = "ElapsedTime" + "," + "furnaceSetTemp" + "," + "FurnaceGetTemp" + "," + "Spot Measured mean" + "," + "Spot Measured Stdev" + "," + "ThermistorTemp" + "," + "BrightMean" + "," + "DarkMean" + "," + "AmbientGetTemp" + "\n"
						hFile.write(arrayOut)

					print("Writing to csv file " + str(fileName) )
					arrayOut = str(elapsedTimeMins) +  "," + str(furnaceSetTemp) + ","  + str(currentfurnace) + "," + str(spotMeanValue) + "," + str(spotNoiseValue) + "," + str(thermistorTempMean) + "," + str(brightMean) + "," + str(darkMean) + "," + str(thisAmbient) +  "\n"
					hFile.write(arrayOut)
